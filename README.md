...
- **☁️ 云存储兼容** - 完美支持 AWS S3、Cloudflare R2 等预签名 URL 下载
- **📊 下载统计** - 可视化统计下载数据，饼状图展示文件类型分布
...
#### 🌐 系统下载器集成 <sup>NEW</sup>
- **浏览器直接调用**：在浏览器中点击下载链接可选择 NyaLoader 打开
- **多协议支持**：支持 HTTP/HTTPS 链接的直接处理
- **丰富 MIME 类型**：支持 zip、rar、7z、pdf、视频、音频等常见格式
- **Intent 共享**：支持从其他应用分享链接到 NyaLoader

#### 📊 下载统计功能 <sup>NEW</sup>
- **可视化统计**：文件类型分布饼状图，直观查看各类文件占比
- **关键指标**：统计总下载量、已完成任务数、成功率等
- **数据持久化**：统计数据保留，应用重启后不丢失
- **一键清除**：支持清除统计数据重新开始

#### ⚡ 下载限速功能 <sup>NEW</sup>
- **速度限制**：支持自定义下载速度上限
- **灵活单位**：KB/s、MB/s 多种单位选择
- **随时切换**：可随时开启/关闭限速
- **后台友好**：适用于需要节省带宽的场景
